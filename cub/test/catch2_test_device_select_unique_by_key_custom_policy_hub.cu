...
C2H_TEST("DispatchUniqueByKey::Dispatch: custom policy hub", "[select_unique_by_key][device]")
{
  ...
  dispatch_t::Dispatch(
    nullptr,
    temp_size,
    cuda::std::to_address(d_keys_in.data()),
    cuda::std::to_address(d_vals_in.data()),
    cuda::std::to_address(d_keys_out.data()),
    cuda::std::to_address(d_vals_out.data()),
    cuda::std::to_address(d_num_selected.data()),
    eq_op_t{},
