...
{
  auto d_keys_in    = cuda::std::to_address(keys_in.data());
  auto d_values_out = cuda::std::to_address(values_out.data());
         ...
         max_num_k_list)
{
  ...
  auto d_values_out_ptr = cuda::std::to_address(values_out_buffer.data());
